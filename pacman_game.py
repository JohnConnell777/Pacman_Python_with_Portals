...
class PacManPortalGame:
    ...
    def __init__(self):
        pygame.init()
        pygame.mixer.music.load('resources/sounds/pacmangreenworldmusic.wav')
        self.screen = pygame.display.set_mode(
            (1600, 900)
        )
        pygame.display.set_caption('PacMan with Portals')
        self.clock = pygame.time.Clock()
        self.score_keeper = ScoreController(screen=self.screen,
                                            sb_pos=((self.screen.get_width() // 5),
                                                    (self.screen.get_height() * 0.965)),
                                            items_image='Cherry-1.png',
        ...
        self.ghost_sound_manager = SoundManager(sound_files=['chasetheghostsahhh.wav', 'pacman_eatghost.wav', 'ghost-std.wav'],
                                                keys=['blue', 'eaten', 'std'],
    ...
    def spawn_ghosts(self):
        files = ['pinkypink.png', 'inkyblue.png', 'clydeorange.png', 'blinkyred.png']
        idx = 0
